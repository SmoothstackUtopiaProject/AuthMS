    ...
    stages {
        ...
        stage('Deploy') {
           steps {
               ...
               sh "aws cloudformation deploy --stack-name UtopiaAuthenticationMS --template-file ./ECSService.yml --parameter-overrides ApplicationName=UtopiaAuthenticationMS ECRepositoryUri=466486113081.dkr.ecr.us-east-1.amazonaws.com/utopiaairlines/authms:$COMMIT_HASH DBUsername=$DB_USERNAME DBPassword=$DB_PASSWORD SubnetID=$SUBNET_ID SecurityGroupID=$SECURITY_GROUP_ID TGArn=$UTOPIA_AUTHMS_TARGETGROUP GmailEmail=$EMAIL_ADDRESS GmailPassword=$EMAIL_PASSWORD JWT=$JSON_WEB_TOKEN_SECRET --capabilities \"CAPABILITY_IAM\" \"CAPABILITY_NAMED_IAM\""
           }
        }
        stage('Cleanup') {
            steps {
