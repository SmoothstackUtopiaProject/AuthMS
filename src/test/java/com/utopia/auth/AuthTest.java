...
public class AuthTest {
  ...
  void testInsertUserCREATED() throws Exception {
    given(userService.insert(any(User.class)))
    ...
    mockMvc
      ...
      .andExpect(jsonPath("$.userFirstName", is("Junit")))
  ...
  void testSendRecoveryEmailOK() throws Exception {
    ...
    given(userService.sendRecoveryEmail(user.getUserEmail()))
      .willReturn(any(MailResponse.class));
    mockMvc
      .perform(
  ...
  void testSendRecoveryEmailCONFLICT() throws Exception {
    ...
    email.put("userEmail", user.getUserEmail());
    given(userService.sendRecoveryEmail(user.getUserEmail()))
      .willThrow(TokenAlreadyIssuedException.class);
    mockMvc
      .perform(
        post("/auth/forgot-password")
          .contentType(MediaType.APPLICATION_JSON)
  ...
  void testVerifyTokenOK() throws Exception {
    ...
    given(userTokenService.verifyToken(user.getUserToken()))
      .willReturn(userToken);
    mockMvc
      .perform(
        post("/auth/forgot-password/verify-token")
          .contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(veriifyToken))
      )
      .andExpect(status().isOk());
  }
  ...
  void testVerifyTokenExpiredToken() throws Exception {
    ...
    veriifyToken.put("recoveryCode", user.getUserToken());
    given(userTokenService.verifyToken(user.getUserToken()))
      .willThrow(ExpiredTokenExpception.class);
    mockMvc
      .perform(
        post("/auth/forgot-password/verify-token")
          .contentType(MediaType.APPLICATION_JSON)
  ...
  void testVerifyTokenExpiredNOTFOUND() throws Exception {
    ...
    veriifyToken.put("recoveryCode", user.getUserToken());
    given(userTokenService.verifyToken(user.getUserToken()))
      .willThrow(TokenNotFoundExpection.class);
    mockMvc
      .perform(
        post("/auth/forgot-password/verify-token")
          .contentType(MediaType.APPLICATION_JSON)
  ...
  void testOnLoginOK() throws Exception {
    ...
    given(userService.findByEmail(authToken.getName()))
    ...
    mockMvc
      .perform(
        get("/auth/login")
          .contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(authToken))
      )
      .andExpect(status().isOk());
  }
